...
public class RiderControler : MonoBehaviour
{
    ...
    // Transform proceduralLeanTarget; // arrasta no Inspector
    ...
    void Update()
    {
        velocity = bike.linearVelocity.magnitude;
        ...
        else
        {
            ...
            // resetar outros parâmetros do freestyle
        ...
        if (!gearControl.OnN)
        {
            ...
            if (velocity > 0.5f)
                anim.SetBool("BikeOnMove", true);
            else
                anim.SetBool("BikeOnMove", false);

        }
        else
        {
            if (VInput > 0 && velocity > 0.7f)
            {
        ...
        if (bikeSystem != null)
        {
            ...
            float currentLeanValue = 0 /*anim.GetFloat("LeanAmount")*/;
